function attachEvents() {
    ...
    async function onClickLoad() {
        ul.innerHTML = '';
        const response = await fetch(url);  //отговора от сървъра, със статус код, и т.н.
        const data = await response.json(); //данните, получени във формата на обект
        ...
        Object.values(data).forEach(x => {
            ...
            const deleteBtn = createElement('button', 'Delete', li); //creating the Delete buttons
    ...
    function createElement(type, text, appender) {
        ...
        const result = document.createElement(type); // създаваме елемент от даденият тип
